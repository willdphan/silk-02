...
Servo rotationServo; // Servo for head movement
int rotationServoPin = 10; // Pin for the head's servo

void MiniKame::rotateServo10(int angle) {
    if (angle >= 0 && angle <= 180) { // Ensure the angle is within a valid range
        rotationServo.write(angle); // Rotate the servo to the specified angle
    } else {
        // Handle invalid angle
        Serial.println(F("Error: Angle out of range. Must be between 0 and 180."));
    }
}
