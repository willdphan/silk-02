...
// Add these prototypes near the top of your firmwareVER_9.cpp file, just after the #include statements
void receiveStop();
void receiveLED();
...
void setup() {
 ...
 //-----
  ...
  SerialCmd.addCommand("R", receiveRotateServo10); 
...
void receiveRotateServo10() {
    sendAck(); // Acknowledge the command

    char *arg = SerialCmd.next(); // Get the next argument (angle) from the command
    if (arg != NULL) {
        int angle = atoi(arg); // Convert angle to integer
        robot.rotateServo10(angle); // Rotate the servo to the specified angle
    } else {
        robot.putMouth(xMouth); // Indicate an error if no angle was provided
        delay(2000);
        robot.clearMouth();
    }

    sendFinalAck(); // Send final acknowledgment
}
